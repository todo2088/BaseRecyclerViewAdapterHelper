...
public abstract class BaseMultiItemQuickAdapter<T extends MultiItemEntity, K extends BaseViewHolder> extends BaseQuickAdapter<T, K> {
     ...
     */
    protected void removeAllChild(@NonNull IExpandable parent, int parentPosition) {
        if (parent.isExpanded()) {
            List<?> childChildes = parent.getSubItems();
            if (childChildes == null || childChildes.size() == 0) return;

            int childSize = childChildes.size();
     ...
     * @param child 子控件实体
     */
    protected void removeDataFromParent(@NonNull T child) {
